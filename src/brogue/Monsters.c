...
void monstersTurn(creature *monst) {
    ...
    if ((monst->creatureState == MONSTER_TRACKING_SCENT
        ...
        if ((monst->bookkeepingFlags & MB_FOLLOWER)
            ...
            && monst->leader->currentHP > fp_trunc(monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst))
...
void monsterDetails(char buf[], creature *monst) {
    ...
    short i, j, combatMath2, playerKnownAverageDamage, playerKnownMaxDamage, commaCount, realArmorValue;
    ...
    if (!rogue.armor || (rogue.armor->flags & ITEM_IDENTIFIED)) {
        ...
        player.info.defense += fp_trunc(10 * strengthModifier(rogue.armor));
        combatMath2 = hitProbability(monst, &player);
    ...
    if ((monst->info.flags & MONST_RESTRICTED_TO_LIQUID) && !cellHasTMFlag(monst->xLoc, monst->yLoc, TM_ALLOWS_SUBMERGING)) {
    ...
    } else if (fp_trunc(monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst)) == 0) {
        ...
        if (monst->info.abilityFlags & MA_POISONS) {
            ...
            for (i = 0; combatMath * (player.poisonAmount + i) < player.currentHP; i++) {
                combatMath += monst->info.damage.upperBound * monsterDamageAdjustmentAmount(monst);
            }
            if (i == 0) {
                ...
                sprintf(newText, "%s has a %i%% chance to poison you and typically poisons for %i turns.\n     ",
                        capMonstName,
                        combatMath2,
                        (int)fp_trunc((monst->info.damage.lowerBound + monst->info.damage.upperBound) * 0.5 * monsterDamageAdjustmentAmount(monst)));
            } else {
            ...
            combatMath = fp_ceil(combatMath); // e.g 2.3 turns to die really means 3 turns, since after 2 turns we'd still have some health left
            if (combatMath < 1) {
                combatMath = 1;
            }
