    ...
    for (feat = 0; feat < blueprintCatalog[bp].featureCount; feat++) {
        ...
        if (feature->flags & (MF_IN_VIEW_OF_ORIGIN | MF_IN_PASSABLE_VIEW_OF_ORIGIN)) {
            ...
            if (feature->flags & MF_IN_PASSABLE_VIEW_OF_ORIGIN) {
                getFOVMask(p->viewMap, originX, originY, max(DCOLS, DROWS), T_PATHING_BLOCKER, 0, false);
            } else {
                getFOVMask(p->viewMap, originX, originY, max(DCOLS, DROWS), (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_VISION), 0, false);
            }
