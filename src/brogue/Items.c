...
#define aggregateGoldUpperBound(d)  (POW_GOLD[d] + 420 * (d))
    ...
    for (i=0; i<numberOfItems; i++) {
        ...
        if (fp((rogue.foodSpawned + foodTable[RATION].strengthRequired / 3) * 4)
            <= (fp_pow(fp(rogue.depthLevel), fp_ratio(135, 100)) + fp(randomDepthOffset)) * foodTable[RATION].strengthRequired * 45/100) {
...
fixpt enchantIncrement(item *theItem) {
    if (theItem->category & (WEAPON | ARMOR)) {
        if (theItem->strengthRequired == 0) {
            ...
            return fp_ratio(35, 10);
        } else {
            return fp_ratio(125, 100);
        }
    } else {
        return FP_ONE;
    }
        ...
        case ARMOR:
            ...
            if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                ...
                if (strengthModifier(theItem)) {
                    sprintf(buf2, ", %s %s %s %s%s%+.2f%s because of your %s strength. ",
                            ...
                            (double)strengthModifier(theItem) / (double)FP_ONE,
                            whiteColorEscape,
            ...
            if (!(theItem->flags & ITEM_EQUIPPED)) {
                if (theItem->category & WEAPON) {
                    ...
                    if (rogue.weapon) {
                        currentDamage = fp_ratio(rogue.weapon->damage.lowerBound + rogue.weapon->damage.upperBound, 2);
                        if ((rogue.weapon->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                            current = fp_round(current * accuracyFraction(netEnchant(rogue.weapon)));
                            currentDamage = fp_mul(currentDamage, damageFraction(netEnchant(rogue.weapon)));
                        } else {
                            current = fp_round(current * accuracyFraction(strengthModifier(rogue.weapon)));
                            currentDamage = fp_mul(currentDamage, damageFraction(strengthModifier(rogue.weapon)));
                        }
                    } else {
                        currentDamage = fp_ratio(player.info.damage.lowerBound + player.info.damage.upperBound, 2);
                    }

                    new = player.info.accuracy;
                    newDamage = fp_ratio(theItem->damage.lowerBound + theItem->damage.upperBound, 2);
                    if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                        new = fp_round(new * accuracyFraction(netEnchant(theItem)));
                        newDamage = fp_mul(newDamage, damageFraction(netEnchant(theItem)));
                    } else {
                        new = fp_round(new * accuracyFraction(strengthModifier(theItem)));
                        newDamage = fp_mul(newDamage, damageFraction(strengthModifier(theItem)));
                    }
                    ...
                    new = theItem->armor;
                    if ((theItem->flags & ITEM_IDENTIFIED) || rogue.playbackOmniscience) {
                        new += fp_round(10 * netEnchant(theItem));
                    } else {
                        new += fp_round(10 * strengthModifier(theItem));
                    }
                    new = max(0, new / 10);
        ...
        case STAFF:
            ...
            if ((theItem->flags & ITEM_IDENTIFIED)  || rogue.playbackOmniscience) {
                sprintf(buf2, "\n\nThe %s has %i charges remaining out of a maximum of %i charges, and%s recovers a charge in approximately %lli turns. ",
                        ...
                        (int)fp_round(fp_div(fp(staffChargeDuration(theItem)), 10 * ringWisdomMultiplier(fp(new)))));
                strcat(buf, buf2);
            } else if (theItem->flags & ITEM_MAX_CHARGES_KNOWN) {
                ...
                switch (theItem->kind) {
                    case STAFF_LIGHTNING:
                        sprintf(buf2, "This staff deals damage to every creature in its line of fire; nothing is immune. (If the staff is enchanted, its average damage will increase by %i%%.)",
                                (int) (100 * (staffDamageLow(enchant + FP_ONE) + staffDamageHigh(enchant + FP_ONE)) / (staffDamageLow(enchant) + staffDamageHigh(enchant)) - 100));
                        break;
                    case STAFF_FIRE:
                    ...
                    case STAFF_BLINKING:
                        sprintf(buf2, "This staff enables you to teleport up to %i spaces. (If the staff is enchanted, this will increase to %i spaces.)",
                                ...
                                staffBlinkDistance(enchant + FP_ONE));
                        break;
                    case STAFF_ENTRANCEMENT:
                        sprintf(buf2, "This staff will compel its target to mirror your movements for %i turns. (If the staff is enchanted, this will increase to %i turns.)",
                                ...
                                staffEntrancementDuration(enchant + FP_ONE));
                        break;
                    case STAFF_HEALING:
                        if (enchant < fp(10)) {
                    ...
                    case STAFF_HASTE:
                        sprintf(buf2, "This staff will cause its target to move twice as fast for %i turns. (If the staff is enchanted, this will increase to %i turns.)",
                                ...
                                staffHasteDuration(enchant + FP_ONE));
                        break;
                    ...
                    case STAFF_DISCORD:
                        sprintf(buf2, "This staff will cause discord for %i turns. (If the staff is enchanted, this will increase to %i turns.)",
                                ...
                                staffDiscordDuration(enchant + FP_ONE));
                        break;
                    case STAFF_CONJURATION:
                        sprintf(buf2, "%i phantom blades will be called into service. (If the staff is enchanted, this will increase to %i blades.)",
                                ...
                                staffBladeCount(enchant + FP_ONE));
                        break;
                    case STAFF_PROTECTION:
                        sprintf(buf2, "This staff will shield a creature for up to 20 turns against up to %i damage. (If the staff is enchanted, this will increase to %i damage.)",
                                ...
                                staffProtection(enchant + FP_ONE) / 10);
                        break;
                    default:
                    ...
                    switch (theItem->kind) {
                        ...
                        case RING_REGENERATION:
                            sprintf(buf2, "\n\nWith this ring equipped, you will regenerate all of your health in %li turns (instead of %li). (If the ring is enchanted, this will decrease to %li turns.)",
                                    ...
                                    (long) (turnsForFullRegenInThousandths(enchant + FP_ONE) / 1000));
                            strcat(buf, buf2);
                            break;
                        ...
                        case RING_WISDOM:
                            sprintf(buf2, "\n\nWhen worn, your staffs will recharge at %i%% of their normal rate. (If the ring is enchanted, the rate will increase to %i%% of the normal rate.)",
                                    (int)fp_round(100 * ringWisdomMultiplier(enchant)),
                                    (int)fp_round(100 * ringWisdomMultiplier(enchant + FP_ONE)));
                            strcat(buf, buf2);
                            break;
                        case RING_REAPING:
            ...
            switch (theItem->kind) {
                ...
                case CHARM_PROTECTION:
                    sprintf(buf2, "\n\nWhen used, the charm will shield you for up to 20 turns for up to %i%% of your total health and recharge in %i turns. (If the charm is enchanted, it will shield up to %i%% of your total health and recharge in %i turns.)",
                            ...
                            charmRechargeDelay(theItem->kind, theItem->enchant1),
                            100 * charmProtection(enchant + FP_ONE) / 10 / player.info.maxHP,
                            charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
                    break;
                case CHARM_HASTE:
                ...
                case CHARM_SHATTERING:
                    sprintf(buf2, "\n\nWhen used, the charm will dissolve the nearby walls up to %i spaces away, and recharge in %i turns. (If the charm is enchanted, it will reach up to %i spaces and recharge in %i turns.)",
                            ...
                            charmRechargeDelay(theItem->kind, theItem->enchant1),
                            charmShattering(enchant + FP_ONE),
                            charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
                    break;
                case CHARM_GUARDIAN:
                    sprintf(buf2, "\n\nWhen used, a guardian will materialize for %i turns, and the charm will recharge in %i turns. (If the charm is enchanted, the guardian will last for %i turns and the charm will recharge in %i turns.)",
                            ...
                            charmRechargeDelay(theItem->kind, theItem->enchant1),
                            charmGuardianLifespan(enchant + FP_ONE),
                            charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
                    break;
                ...
                case CHARM_NEGATION:
                    sprintf(buf2, "\n\nWhen used, the charm will negate all magical effects on the creatures in your field of view and the items on the ground up to %i spaces away, and recharge in %i turns. (If the charm is enchanted, it will reach up to %i spaces and recharge in %i turns.)",
                            ...
                            charmRechargeDelay(theItem->kind, theItem->enchant1),
                            charmNegationRadius(enchant + FP_ONE),
                            charmRechargeDelay(theItem->kind, theItem->enchant1 + 1));
                    break;
                default:
                    break;
...
short displayedArmorValue() {
    if (!rogue.armor || (rogue.armor->flags & ITEM_IDENTIFIED)) {
        ...
        short hi = armorTable[rogue.armor->kind].range.upperBound;
        short lo = armorTable[rogue.armor->kind].range.lowerBound;
        return fp_round(fp_ratio(hi + lo, 2 * 10) + strengthModifier(rogue.armor));
    }
}
...
short getLineCoordinates(short listOfCoordinates[][2], const short originLoc[2], const short targetLoc[2]) {
    ...
    for (i=0; i<= 1; i++) {
        targetVector[i] = fp(targetLoc[i] - originLoc[i]);
        if (targetVector[i] < 0) {
            targetVector[i] *= -1;
    ...
    targetVector[0] = fp(targetVector[0]) / largerTargetComponent;
    targetVector[1] = fp(targetVector[1]) / largerTargetComponent;

    do {
        for (i=0; i<= 1; i++) {

            currentVector[i] += fp_round(targetVector[i]);
            error[i] += (targetVector[i] == FP_ONE ? 0 : targetVector[i]);
    ...
    if (monst && !(monst->bookkeepingFlags & MB_SUBMERGED)) {
        ...
        switch(theBolt->boltEffect) {
            ...
            case BE_DAMAGE:
                ...
                } else if (inflictDamage(caster, monst, staffDamage(fp(theBolt->magnitude)), theBolt->backColor, false)) {
                    // killed monster
            ...
            case BE_SLOW:
                ...
                break;
            case BE_HASTE:
                haste(monst, staffHasteDuration(fp(theBolt->magnitude)));
            ...
            case BE_POISON:
                if (!(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))) {
                    addPoison(monst, staffPoison(fp(theBolt->magnitude)), 1);
                    if (canSeeMonster(monst)) {
            ...
            case BE_ENTRANCEMENT:
                if (monst == &player) {
                    ...
                    monst->status[STATUS_CONFUSED] = staffEntrancementDuration(fp(theBolt->magnitude));
                    ...
                    monst->status[STATUS_ENTRANCED] = monst->maxStatus[STATUS_ENTRANCED] = staffEntrancementDuration(fp(theBolt->magnitude));
                    wakeUp(monst);
                    if (canSeeMonster(monst)) {
            ...
            case BE_DISCORD:
                if (!(monst->info.flags & (MONST_INANIMATE | MONST_INVULNERABLE))) {
                    monst->status[STATUS_DISCORDANT] = monst->maxStatus[STATUS_DISCORDANT] = max(staffDiscordDuration(fp(theBolt->magnitude)),
                ...
                break;
            case BE_SHIELDING:
                if (staffProtection(fp(theBolt->magnitude)) > monst->status[STATUS_SHIELDED]) {
                    monst->status[STATUS_SHIELDED] = staffProtection(fp(theBolt->magnitude));
                }
...
void detonateBolt(bolt *theBolt, creature *caster, short x, short y, boolean *autoID) {
    ...
    switch(theBolt->boltEffect) {
        case BE_OBSTRUCTION:
            ...
            fixpt t = fp_ipow(fp_ratio(8, 10), min(40, theBolt->magnitude)); // 0.8 ^ magnitude
            feat.probabilityDecrement = max(1, fp_round(75 * t));
            ...
            break;
        case BE_CONJURATION:
            for (i = 0; i < staffBladeCount(fp(theBolt->magnitude)); i++) {
...
boolean zap(short originLoc[2], short targetLoc[2], bolt *theBolt, boolean hideDetails) {
    ...
    if (boltColor) {
        for (i=0; i<initialBoltLength; i++) {
            ...
            boltLightRadius = fp_round(50 * fp_mul(fp(3) + fp_ratio(theBolt->magnitude * 4, 3), fp_ratio(initialBoltLength - i, initialBoltLength)));
...
boolean hitMonsterWithProjectileWeapon(creature *thrower, creature *monst, item *theItem) {
    ...
    if (thrownWeaponHit) {
        damage = monst->info.flags & (MONST_IMMUNE_TO_WEAPONS | MONST_INVULNERABLE) ? 0 :
                  fp_round(randClump(theItem->damage) * damageFraction(netEnchant(theItem)));
        ...
        for (i = 0; i < numCells; i++) {
            ...
            if (pmap[x][y].flags & DISCOVERED) {
                ...
                if ((tFlags & T_LAVA_INSTA_DEATH)
                    ...
                    possibleDeath = true;
                } else if (i >= staffBlinkDistance(fp(2)) - 1) {
...
void recalculateEquipmentBonuses() {
    ...
    if (rogue.weapon) {
        ...
        player.info.damage.lowerBound = fp_round(player.info.damage.lowerBound * damageFraction(enchant));
        player.info.damage.upperBound = fp_round(player.info.damage.upperBound * damageFraction(enchant));
    ...
    if (rogue.armor) {
        theItem = rogue.armor;
        enchant = netEnchant(theItem);
        enchant -= fp(player.status[STATUS_DONNING]);
        player.info.defense = fp_round(fp(theItem->armor) + enchant * 10);
...
void updatePlayerRegenerationDelay() {
    ...
    turnsForFull = turnsForFullRegenInThousandths(fp(rogue.regenerationBonus));
